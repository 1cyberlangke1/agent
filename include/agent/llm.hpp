...
/// 统一思考等级。用户只传统一等级，引擎内部映射到各家格式。
/// 枚举值必须按序 0..6（Off/Minimal/Low/Medium/High/XHigh/Max），
/// 用作数组下标访问 thinking_level_map。
...
/// 等级数。模型表三结构 thinking_level_map 的维度，static_assert 与枚举同步：
...
struct BuiltinModel {
    ...
    /// 统一 ThinkingLevel → 厂商原生值映射；nullopt = 该等级不支持。
    /// 固定数组按下标访问（枚举按序 0..6），无 map 树。
    /// 值统一为字符串形态（"high" / "8192"），指向静态字面量。
...
/// 全局模型注册表。**纯静态类**：禁实例化，全 static 成员，进程生命周期。
...
public:
    ...
    /// 运行时注册/覆盖模型（同 id 覆盖内置或已有动态项）。线程安全。
...
/// 把用户请求的思考等级收敛到模型支持的范围内（从低向高找最近支持档）。
/// @param model  目标模型
/// @param level  用户请求的等级
...
/// 工具参数增量。delta 阶段只给 JSON 字符串增量，不 parse——
...
/// 统一流事件。每个事件 = 类型判别 + 变体载荷，不用「一堆 optional 字段」。
struct StreamEvent {
    enum class Type {
        TextDelta,
        ThinkingDelta,
    ...
    /// 类型判别从 variant 下标推导——单一真相源，不存冗余字段
...
/// 基于 asio::experimental::basic_concurrent_channel 的薄包装。
...
public:
    ...
    /// 生产端结束：发送完毕后关闭通道。消费端 receive 收到 channel_closed 错误。
    ...
    /// 消费端同步取（非阻塞）。队列空 → nullopt；否则返回事件或关闭错误。
...
/// OpenAI 协议能力位（数据开关，constexpr static 类型策略）。
...
/// DeepSeek 思考行为：thinking 内容走 reasoning_content，须补空字段。
...
/// OpenAI 协议 Provider。实现见 src/provider_openai.cpp。
...
public:
    ...
    /// AsyncStream 按值移动进入（channel 可移动），生产协程生命周期由引擎保证，
    /// 调用方不持有悬空引用。
...
/// DeepSeek Provider：复用 OpenAI 协议引擎 + DeepSeek 思考策略。
...
/// Anthropic Messages 协议 Provider。实现见 src/provider_anthropic.cpp。
...
/// Gemini GenerateContent 协议 Provider。实现见 src/provider_gemini.cpp。
